...
#import "BPZFPlayerControlView.h"
...
@interface BPZFPlayerControlView () <UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) BPPlayerConfig* config;

@end

@implementation BPZFPlayerControlView

- (instancetype)init
{
    self = [super init];
    if (self) {
        [self initControlles:[[BPPlayerConfig alloc] init]];
    }
    return self;
}
...
- (instancetype)initWithConfig:(BPPlayerConfig*)config
{
    self = [super init];
    if (self) {
        [self initControlles:config];
    }
    return self;
}

- (void)initControlles:(BPPlayerConfig*)config
{
    _config = config;
    [self addSubview:self.placeholderImageView];
    [self addSubview:self.topImageView];
    [self addSubview:self.bottomImageView];
    if (!config.isHideStartButton) {
        [self.bottomImageView addSubview:self.startBtn];
    }
    [self.bottomImageView addSubview:self.currentTimeLabel];
    [self.bottomImageView addSubview:self.progressView];
    [self.bottomImageView addSubview:self.videoSlider];
    if (!config.isHideFullScreenButton) {
    ...
    if (config.isLikeButtonShow) {
        [self.topImageView addSubview:self.likeBtn];
    }
    
    [self addSubview:self.activity];
    [self addSubview:self.repeatBtn];
    if (!config.isHidePlayButton) {
...
- (void)makeSubViewsConstraints
{
    ...
    [self.topImageView mas_makeConstraints:^(MASConstraintMaker* make) {
        ...
        if (_config.isInTop) {
            make.height.mas_equalTo([SDVersion deviceSize] == Screen5Dot8inch ? 94 : 70);
        } else {
    ...
    if (_config.isLikeButtonShow) {
        [self.likeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.topImageView).offset(-5);
    ...
    if (!_config.isHideFullScreenButton) {
...
{
    if (self.block) {
        self.block(PlayOperationTypeBackAction);
    }
    return;
...
- (UILabel*)titleLabel
{
    if (!_titleLabel) {
        ...
        _titleLabel.font = [UIFont systemFontOfSize:17.0 weight:UIFontWeightLight];
    }
    return _titleLabel;
}
...
- (UIButton*)backBtn
{
    if (!_backBtn) {
        ...
        [_backBtn setImage:[UIImage bp_imageNamed:@"backImage"] forState:UIControlStateNormal];
...
- (UIButton*)likeBtn
{
    if (!_likeBtn) {
        ...
        [_likeBtn setImage:[UIImage bp_imageNamed:@"happysing_icon_thumpup_white"] forState:UIControlStateNormal];
        WeakSelf;
        [_likeBtn addBlockForControlEvents:UIControlEventTouchUpInside block:^(UIButton * sender) {
            if (weakSelf.block) {
                weakSelf.block(PlayOperationTypeLikeAction);
            }
        }];
    }
    return _likeBtn;
...
- (ASValueTrackingSlider*)videoSlider
{
    if (!_videoSlider) {
        ...
        if (_config.isSliderGestureDisabled) {
...
- (void)zf_playerShowControlView
{
    ...
    [self zf_playerCancelAutoFadeOutControlView];
    [UIView animateWithDuration:ZFPlayerControlBarAutoFadeOutTimeInterval
        animations:^{
            [self showControlView];
        }
...
- (void)zf_playerCurrentTime:(NSInteger)currentTime totalTime:(NSInteger)totalTime sliderValue:(CGFloat)value
{
    ...
    if (self.progress_block) {
        self.progress_block(currentTime, totalTime, value);
    }
}
...
- (void)zf_playerPlayEnd
{
    ...
    self.bottomProgressView.alpha = 0;
    if (self.block) {
        self.block(PlayOperationTypePlayFinished);
    }
}
